...
const login = async () => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const findEmployeeRol = async() => {
                await fetch(`http://localhost:8080/api/rol/${dto.user}`, {
                    method: 'GET',
                    headers: {
            ...
            if(rol==1){
                ...
                window.location.replace('http://127.0.0.1:5501/src/view/RAPE/verProyecto.html');
            }else if(rol==3){
                window.location.replace('http://127.0.0.1:5501/src/view/RD/verProyecto.html');
            }else if(rol==4){
                window.location.replace('http://127.0.0.1:5501/src/view/AP/verProyecto.html');
            }
